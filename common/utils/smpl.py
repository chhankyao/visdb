...
class SMPL(object):
    def __init__(self):
        ...
        head_top_onehot = np.array([1 if i == 411 else 0 for i in range(self.joint_regressor.shape[1])], dtype=np.float32).reshape(1,-1)
        
        if cfg.add_head_top:
            self.face_kps_vertex = (331, 2802, 6262, 3489, 3990, 411) # mesh vertex idx
            self.joint_regressor = np.concatenate((self.joint_regressor, nose_onehot, left_eye_onehot, right_eye_onehot, 
                                                   left_ear_onehot, right_ear_onehot, head_top_onehot))
            self.joint_num = 30 # original: 24. manually add nose, L/R eye, L/R ear, head_top
            ...
            self.joint_regressor = np.concatenate((self.joint_regressor, nose_onehot, left_eye_onehot, right_eye_onehot, 
                                                   left_ear_onehot, right_ear_onehot))
